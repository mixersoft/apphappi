...
class Server
  ...
  constructor: (@options) ->
    ...
    @app.use('/data', (req, res, next)=>
      console.log "### MODIFIED: path=" + req.path + ", headers.origin=" + req.headers.origin
      if !req.headers.origin
        return next();
      console.log "### > Server CORS Access-Control-Allow-Origin " + (req.headers.origin || req.headers.host) + "  #####"
